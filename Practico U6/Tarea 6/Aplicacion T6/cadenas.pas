...
uses
 ...
 cad=class
   ...
   public
         ...
         function Intercambiar():string;                  //Estas funciones estan mal hechas, las hice cuando estaba en introduccion a la informatica, ignorarlas
         ...
         function GetPos(pos:integer):char;

         procedure Encriptar();                               // Estas son las que use
...
function cad.GetPos(pos: integer): char;
begin
  result:=cadenita[pos];
end;

procedure cad.Encriptar();
var t,posi:integeR;
begin
  t:=length(cadenita);
  ...
  for posi:=1 to t do begin
      if cadenita[posi]='A' then begin
         cadenita[posi]:='!';
      end;

      if cadenita[posi]='E' then begin
         cadenita[posi]:='*';
      end;

      if cadenita[posi]='I' then begin
         cadenita[posi]:='#';
      end;

      if cadenita[posi]='O' then begin
         cadenita[posi]:='$';
      end;

      if cadenita[posi]='U' then begin
         cadenita[posi]:='%';
      end;
